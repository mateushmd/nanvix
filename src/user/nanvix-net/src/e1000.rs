...
impl<K: KernelFunctions> E1000Device<K> {
    ...
    pub fn set_hardware_addr(&mut self, address: EthernetAddress) {
        ...
        let ral = u32::from(a) | (u32::from(b) << 8) | (u32::from(c) << 16) | (u32::from(d) << 24);
...
impl<K: KernelFunctions> Drop for E1000Device<K> {
    fn drop(&mut self) {
        ...
        self.kfn
            .dma_free(self.tx_buffers.vaddr, self.tx_buffers.pages);
        self.kfn
            .dma_free(self.rx_buffers.vaddr, self.rx_buffers.pages);
    }
}
...
mod tests {
    ...
    fn receive_returns_completed_frame_and_reclaims_descriptor() {
        ...
        unsafe {
            let buffer =
                slice::from_raw_parts_mut(device.rx_buffer_ptr(0) as *mut u8, payload.len());
